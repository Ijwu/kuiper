...
namespace kuiper.Plugins
{
    ...
    public abstract class BasePlugin : IPlugin
    {
        ...
        protected BasePlugin(ILogger logger, WebSocketConnectionManager connectionManager)
        {
        ...
        protected async Task<long?> GetSlotForConnectionAsync(string connectionId)
        {
            ...
            if (!slotId.HasValue)
            {
                Logger.LogDebug("Connection {ConnectionId} has no mapped slot; ignoring packet.", connectionId);
            }
            return slotId;
        }
